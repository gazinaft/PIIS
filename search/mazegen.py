...
class Map:
    ...
    def get_tile(self,x,y):
        if self.xy_valid(x,y): return self.tiles[x+y*self.w]
        return None

    # place wall at x,y
    def place_wall(self,x,y):
        if self.xy_valid(x,y):
            self.tiles[x+y*self.w] = WALL

    def block_is_valid(self,x,y):
        return all(self.get_tile(x+dx,y+dy) == WALL for dy in range(1,3) for dx in range(1,3))

    # place 2x2 block 
    def add_wall_block(self,x,y):
        self.place_wall(x+1,y+1)
        self.place_wall(x+2,y+1)
        self.place_wall(x+1,y+2)
        self.place_wall(x+2,y+2)
        ...
        for y in range(self.h):
            for x in range(self.w):
                if (x,y) in self.pos_list:
                    if any(self.get_tile(x-1,y+y0)==WALL for y0 in range(4)): self.add_connection(x,y,1,0)
                    if any(self.get_tile(x+4,y+y0)==WALL for y0 in range(4)): self.add_connection(x,y,-1,0)
                    if any(self.get_tile(x+x0,y-1)==WALL for x0 in range(4)): self.add_connection(x,y,0,1)
                    if any(self.get_tile(x+x0,y+4)==WALL for x0 in range(4)): self.add_connection(x,y,0,-1)
        ...
        def expand(x,y):
            ...
            if src in self.connections:
                for x0,y0 in self.connections[src]:
                    if not self.block_is_valid(x0,y0):
                        count += 1
...
if __name__ == "__main__":
    ...
    for line in str(tileMap).splitlines():
        ...
        s = line[:14]
        res = s+s[::-1]
        if count >= mid and not done and can_pacman(res):
            midw = int(len(res) / 2)
            temp = list(res)
            temp[midw] = 'P'
            res = ''.join(temp)
